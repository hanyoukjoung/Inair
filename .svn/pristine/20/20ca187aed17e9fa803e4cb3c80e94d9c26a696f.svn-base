...
function getLastDate(dateVal){
	...
	switch((dateVal.getMonth()+1)){
		...
		case 11:
			...
			break;
		case 2:
			if((dateVal.getFullYear() % 4 == 0 && dateVal.getFullYear() % 100 != 0) || (dateVal.getFullYear() % 400 == 0)){
				day = 29;
			}else{
				day = 28;
			}
...
//하위 직종 selectBox 셋팅
function getLowPROList(value, appendId){
	$.ajax({
		...
		,success : function(result){
							$.each(result, function(index, item){
								var changeOption = $('<option value="'+ item.lowpro_num +'">'+ item.lowpro_name +'</option>');
...
//하위 업종 selectBox 셋팅
function getLowIndustryList(value, appendId){
	$.ajax({
		...
		,success : function(result){
						$.each(result, function(index, item){
							var changeOption = $('<option value="'+item.lwinduty_num+'">'+item.lwinduty_name+'</option>');
							$(appendId).append(changeOption);
						});
				}
	});
