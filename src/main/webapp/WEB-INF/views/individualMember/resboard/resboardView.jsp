...
<script type="text/javascript">
	$(function(){
		...
		$('#listBTN').click(function(){
			...
			location.href='/resboard/indvdlResboardHome.do?search_keyword='+search_keyword+'&search_keycode=' + search_keycode;
		});
		        ...
		        <label for="content">comment</label>
			        <form name="commentInsertForm">
			            <div class="input-group">
			               <input type="hidden" name="res_num" value="${resboardInfo.res_num}"/>
	...
	$.ajax({
	       ...
	       ,success : function(result){
	           ...
	           var a =''; 
	              ...
	              if(result.resboardList[i].indvdl_id=='${LOGIN_INDVDLMEMINFO.indvdl_id}'){
	               a += '<a  onclick="commentUpdate('+result.resboardList[i].res_num+',\''+result.resboardList[i].res_cn+'\',\''+result.resboardList[i].indvdl_id+'\');"> 수정 </a>';
...
function commentUpdate(res_num,res_cn,indvdl_id) {
...
function commentReplyReplyInsert(res_depth,res_num,res_sn){
	var res_cn = $("input[name='content_"+res_num+"']").val();
	 var insertData = {'indvdl_id' : '${LOGIN_INDVDLMEMINFO.indvdl_id}','com_id':'${resboardInfo.com_id}','res_group' : '${resboardInfo.res_group }', 'res_sj' : '댓글','res_sn' : res_sn, 'res_depth' : res_depth,'res_cn' :res_cn, 'res_num' : '${resboardInfo.res_num }'};
	$.ajax({		 
