		    ...
		    <tbody id="characterTbody">
		    	<c:forEach items="${resumeList}" var="resumeInfo" >
			    	<c:forEach items="${resumeInfo.portfolioList}" var="portfolioInfo">	
	       				<tr>
	       					<td>
	       						<input type="button" class="portBtn" value="${portfolioInfo.port_filename}" onclick="javascript:location.href='/individualMember/jobOffer/portfolioDownload.do?port_filesavename=${portfolioInfo.port_filesavename}&port_filename=${portfolioInfo.port_filename}'" />
	       					</td>
	       				</tr>
	     			</c:forEach>
     			</c:forEach>
