			...
			$.ajax({
				...
				,success : function(result) {
							if(result[0].resultFlag == "true") {
								...
								$.each(result, function(index, item) {
									if(index > 0) {
										list += '<div id="' + item.corp_code + '" value="' + item.corp_code 
											 + '" class="corpNameP" onclick="clickedCompanyName(this.id)">' 
											 + item.corp_name + '</div>';
									}
								});
...
// 								corpCode = cropCodeValue;
	...
	$('#searchSubmit').click(function() {
		...
		var corp_add = $('#addressInput').val();
		var corp_lowPro = $('.lowProButton').val();
		location.href = '/companySearch/companySearchHome.do?corp_name=' + corp_name + '&corp_add=' + corp_add + '&corp_lowPro=' + corp_lowPro;
	});
...
// 													+ '"style="font-size: 13px; text-align: center; height: 30px; width: 180px; background-color: #f8f9fa; border-radius:5px; margin: 5px;"'
...
// 선택에 따른 하위 업종 불러오기
// function getLowIndustry(value, appendid) {
// 	$.ajax({
// 		url : contextPath + '/individualMember/lowIndustryList.do'
// 		, dataType : 'json'
// 		, data : { upinduty_num : value }
// 		, error : function(result) {
// 					alert(result.status);
// 				}
// 		, success : function(result) {
// 					$.each(result, function(index, item) {
// 						var changeOption = $('<button class="lowProButton" id="lowInduty' + item.lwinduty_num
// 											+ '"value="' + item.lwinduty_num
// 											+ '"onclick="clickedLowIndustry(this.id)">'
// 											+ item.lwinduty_name + '</button>');
// 						$(appendid).append(changeOption);
// 					});
// 				}
// 	});
...
// onclick 업종
// function selectLowIndutyFunction(thisId) {
// 	$('#nextId').empty();
	
// 	getLowIndustry($('#' + thisId).val(), '#nextId');
// }

// function clickedLowIndustry(thisLowId) {
// 	$('#proInput').val($('button[id="' + thisLowId + '"]').text());
// 	$('#professionSelectTable').slideUp();
// }
...
<body style="position: relative;">
	<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
		<div class="container-fluid px-md-4	">
			...
			<c:if test="${not empty LOGIN_INDVDLMEMINFO}">
				<div class="collapse navbar-collapse" id="ftco-nav">
					<ul class="navbar-nav ml-auto">
						<li class="nav-item"><a href="/myPage/myPageHome.do" class="nav-link" style="font-size: 20px;">${LOGIN_INDVDLMEMINFO.indvdl_name}<span style="font-size: 13px;">(${LOGIN_INDVDLMEMINFO.indvdl_id})</span> 님</a></li>
						<li class="nav-item"><a href="${individualMemberURL}/logout.do" class="nav-link" style="font-size: 20px;">로그아웃</a></li>
					</ul>
				</div>
			</c:if>
	...
	<!-- END nav -->
	<div id="imgDiv" class="hero-wrap img" style="background-image: url(${pageContext.request.contextPath}/images/bg_1.jpg);">
		<div class="overlay"></div>
		<div class="container">
			<div class="row d-md-flex no-gutters slider-text align-items-center justify-content-center">
				<div class="col-md-10 d-flex align-items-center ftco-animate">
					<div class="text text-center pt-5 mt-md-5">
						<p class="mb-4">The Eassiest Way to Get Your New Job</p>
						<h1 class="mb-5">새로운 직업을 얻는 가장 쉬운 방법</h1>
									...
									<div class="block-18">
										<div class="text d-flex">
											...
											<div class="desc text-left">
												<strong class="number" data-number="${connectCount}"></strong>
												<span>방문자수</span>
											</div>
										</div>
									</div>
								...
								<div class="col-md-4 d-flex justify-content-center counter-wrap ftco-animate">
									<div class="block-18 text-center">
										<div class="text d-flex">
											...
											<div class="desc text-left">
												<strong class="number" data-number="${jobOfferCount}"></strong> <span>구인글 등록수</span>
											</div>
										</div>
									</div>
								</div>
								<div class="col-md-4 d-flex justify-content-center counter-wrap ftco-animate">
									<div class="block-18 text-center">
										<div class="text d-flex">
											...
											<div class="desc text-left">
												<strong class="number" data-number="${indvdlMemCount}"></strong> <span>구직자 등록수</span>
											</div>
										</div>
									</div>
								</div>
													...
													<div class="col-md mr-md-2">
														<div class="form-group">
															<div class="form-field">
																...
																<input type="text" id="corp_name" class="form-control" name="corp_name" placeholder="검색어 입력">
															</div>
														</div>
													</div>
													<div class="col-md mr-md-2">
														<div class="form-group">
															<div class="form-field">
																<div class="select-wrap">
																	...
																	<input type="text" id="proInput" class="form-control" name="corp_lowPro" placeholder="직종 선택">
<!-- 																	<input type="text" id="industryInput" class="form-control" name="lwinduty_num" placeholder="업종 선택"> -->
																</div>
															</div>
														</div>
													</div>
														...
														<div class="form-group">
															<div class="form-field">
																<div class="select-wrap">
																	...
																	<input type="text" id="addressInput" class="form-control" name="corp_add" placeholder="지역 선택">
																</div>
															</div>
														</div>
													</div>
...
<!-- 																<button type="submit" id="searchSubmit" class="form-control btn btn-primary">검색</button> -->
															</div>
														</div>
													</div>
												</div>
...
<!-- 												<tr> -->
<!-- 													<td style="width: 135px;"> -->
<%-- 														<c:forEach items="${upIndustryList}" var="indutyInfo" varStatus="stat" > --%>
<%-- 															<c:if test="${stat.index <= 4 }"> --%>
<!-- 																<div> -->
<%-- 																	<button id="indutyButton${indutyInfo.upinduty_num}" class="indutyButton" value="${indutyInfo.upinduty_num}" onclick="selectLowIndutyFunction(this.id)">${indutyInfo.upinduty_name }</button> --%>
<!-- 																</div> -->
<%-- 															</c:if> --%>
<%-- 														</c:forEach> --%>
<!-- 													</td> -->
<!-- 													<td style="width: 135px;"> -->
<%-- 														<c:forEach items="${upIndustryList}" var="indutyInfo" varStatus="stat"> --%>
<%-- 															<c:if test="${stat.index >= 5 || stat.index <= stat.end}">  --%>
<!-- 																<div> -->
<%-- 																	<button id="indutyButton${indutyInfo.upinduty_num}" class="indutyButton" value="${indutyInfo.upinduty_num}" onclick="selectLowIndutyFunction(this.id)">${indutyInfo.upinduty_name }</button> --%>
<!-- 																</div> -->
<%-- 															</c:if> --%>
...
<!-- 						<div id="test" style="background-color: blue; position: absolute; width: 300px; height: 400px; top: 637px; left: 25%;"></div> -->
					</div>
				</div>
			</div>
		</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="category-wrap">
						<div class="row no-gutters">
							<div class="col-md-2" onclick="javascript:location.href='/companySearch/companySearchHome.do'">
								<div class="top-category text-center no-border-left">
									...
									<p>
										<span class="number">143</span> <span>Open position</span>
									</p>
								</div>
							</div>
							<div class="col-md-2" onclick="javascript:location.href='${pageContext.request.contextPath}/AIIMT/AIIMTHome.do'">
							...
							<div class="col-md-2" onclick="javascript:location.href='/character/characterHome.do'">
								<div class="top-category text-center">
									<h3>
										<p>인성역량검사</p>
									</h3>
							...
							<div class="col-md-2" onclick="javascript:location.href='${individualMemberURL}/resumeHome.do'">
								<div class="top-category text-center">
									<h3>
										<p>이력서관리</p>
									</h3>
							...
							<div class="col-md-2" onclick="javascript:location.href='/mentorboard/mentorboardHome.do'">
								<div class="top-category text-center">
									<h3>
										<p>커뮤니티</p>
									</h3>
							...
							<div class="col-md-2" onclick="javascript:location.href='/myPage/myPageHome.do'">
								<div class="top-category text-center">
									<h3>
										<p>개인정보관리</p>
									</h3>
				...
				<div class="col-md-12 text-center">
					<p>
						...
						All rights reserved | This template is made with <i
							...
							href="${adminURL}/login/adminLoginForm.do" target="_blank">inAIR</a>
						<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
					</p>
				</div>
			</div>
	...
	<df-messenger 
		intent="WELCOME" 
		chat-title="inAIR" 
		agent-id="09d04d09-e8af-4b75-afce-985bf07b6f64" 
		language-code="ko">
	</df-messenger>
