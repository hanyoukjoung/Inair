...
<script type="text/javascript">
  $(function(){
	...
	$('#fboardListTBY tr').click(function(){
		...
		location.href='/freeboard/freeboardView.do?freeboard_num='+freeboard_num+'&rnum='+rnum
...
<body>
	<div class="container" style="margin-top: 5%; margin-left:25%; width:60%" >
	  <h2>취업자유게시판</h2>
			    ...
			    <c:forEach items="${freeboardList}" var="freeboardInfo">
			    	<c:if test="${freeboardInfo.freeboard_depth < 1}">
				      <tr>
				        ...
				        <td><input type="hidden" value="${freeboardInfo.freeboard_group}"/>${freeboardInfo.freeboard_sj}</td>
				        <td>${freeboardInfo.indvdl_id}</td>
				        <td>${freeboardInfo.freeboard_rgsde.substring(0,10)}</td>
				        <td>${freeboardInfo.freeboard_hit}</td>
				      </tr>
			       </c:if>
			    </c:forEach>
			 </c:if>
	...
	<div>
	  ...
	  <c:if test="${!empty LOGIN_INDVDLMEMINFO.indvdl_id}">
		<button type="button" id="btnform" class="btn btn-primary py-2" style="margin-left: 78%">게시글 등록</button>
	  </c:if>	   	
		  	<form name="searchForm" action="/freeboard/freeboardHome.do" method="post" class="search-job" style="width: 25%;margin-left: 60%;margin-top: 2%">
