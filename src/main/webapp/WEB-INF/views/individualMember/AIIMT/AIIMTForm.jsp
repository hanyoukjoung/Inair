...
function finishTest(){
	var message = [
		"모든 테스트를 통과했습니다.",
		"이제 본격적으로 AI모의면접을 시작하겠습니다.(5초 후 시작됩니다.)"
	];
...
function mainStart(){
	...
	var message = [
        "Q1. 당신의 장단점을 말씀해주세요.(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q2. 가장 존경하는 인물과 이유를 말씀해주세요.(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q3. 성공을 위해 가장 필요한 것과 이유를 말씀해주세요.(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q4. 당신이 가장 행복했던 순간은 언제였나요?(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q5. 당신이 가장 슬펐던 순간은 언제였나요?(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q6. 살면서 가장 힘들었던 순간은 언제였나요?(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q7. 살면서 가장 후회하는 순간은 언제였나요?(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q8. 동료와 의견충돌이 있을때 어떻게 해결할 것 인가요?(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q9. 인간관계에 있어서 가장 중요한 것과 이유를 말씀해주세요.(답변시간 30초)",
		"쉬는 타임입니다.(5초)",
		"Q10. 자신이 가지고 있는 역량은 무엇입니까?(답변시간 30초)",
		"모든면접이 완료되었습니다. 수고하셨습니다."
	];
	
	$('#myBar').css('width', '0%');
	
	if(mainCount%2 == 0){ // 메시지 홀수일때
	...
	}else if(mainCount%2 != 0){	// 메시지 짝수일때
		$('#message').text(message[mainCount]);
		move(20);
		var messageInterval = setInterval(function(){
			mainCount++;
	...
	}else if(mainCount == 19){	// 마지막 메시지일때
		$('#message').text(message[mainCount]);
		move(20);
		var messageInterval = setInterval(function(){
			finishInterview();
...
function voiceAnalysis() {
	...
	var set_voice_calm  	= Math.floor(((voice_calm/total) 	  * 100) * 100) / 100;
	var set_voice_anger 	= Math.floor(((voice_anger/total) 	  * 100) * 100) / 100;
	var set_voice_happiness = Math.floor(((voice_happiness/total) * 100) * 100) / 100;
	var set_voice_sadness 	= Math.floor(((voice_sadness/total)   * 100) * 100) / 100;
	var set_voice_vitality	= Math.floor(((voice_vitality/total)  * 100) * 100) / 100;
