...
<script type="text/javascript">
	...
	$(function(){
		...
		$('tr .comMemTableTd').click(function(){
			var comMemberViewForm = document.createElement("form");
			comMemberViewForm.setAttribute("charset", "UTF-8");
			comMemberViewForm.setAttribute("method", "post"); 
			comMemberViewForm.setAttribute("action", "${adminMemberManagementURL}/comMemberViewForm.do"); 
			comMemberViewForm.setAttribute("name", "comMemberViewForm"); 
			
			document.body.appendChild(comMemberViewForm);
			
			var comId = $(this).parent().find('.comIdTd').text();
			var $comMemId = $('<input type="hidden" name="com_id" value="'+ comId +'"/>');
			
			$('form[name=comMemberViewForm]').append($comMemId);
			
			comMemberViewForm.submit();
		});
			              	...
			              	<c:if test="${!empty companyMemList}">
				              	<c:forEach items="${companyMemList}" var="companyMemInfo">
					                <tr class="gradeX">
					                  ...
					                  <td class="comMemTableTd">${companyMemInfo.com_secsn}</td>
					                </tr>
				                </c:forEach>
			                </c:if>
			              </tbody>
